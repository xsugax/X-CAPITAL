...
const GOD_ADMIN_PW_HASH = '6e3a6c3f8e4b2a1d9c8f7e6d5b4a3c2e1f0d9c8b7a6e5d4c3b2a1f0e9d8c7b6'; // placeholder, will be computed
...
interface AuthState {
  ...
  logout: () => void;
  registerUser: (userData: { firstName: string; lastName: string; email: string; password: string }) => Promise<{ success: boolean; error?: string }>;
  loginUser: (email: string, password: string) => Promise<{ success: boolean; error?: string }>;
  getAllUsers: () => User[];
  updateUserById: (userId: string, updates: Partial<User>) => void;
  deleteUserById: (userId: string) => void;
  createUserAsAdmin: (userData: { firstName: string; lastName: string; email: string; password: string; role?: string; tier?: string }) => Promise<{ success: boolean; error?: string }>;
  ...
  persist(
    (set, get) => ({
      ...
      loginUser: async (email, password) => {
        ...
        if (email.toLowerCase() === 'admin@xcapital.io' && pwHash === godAdminHash) {
          const adminUser = state.registeredUsers.find(u => u.email === 'admin@xcapital.io') || {
            ...GOD_ADMIN_USER,
          ...
          const exists = state.registeredUsers.some(u => u.email === 'admin@xcapital.io');
          const updatedUsers = exists
            ? state.registeredUsers.map(u => u.email === 'admin@xcapital.io' ? { ...u, lastLogin: new Date().toISOString() } : u)
            : [...state.registeredUsers, { ...adminUser, lastLogin: new Date().toISOString() }];
        ...
        const foundUser = state.registeredUsers.find(u => u.email.toLowerCase() === email.toLowerCase());
        if (!foundUser) return { success: false, error: 'No account found with this email.' };
        if (foundUser.passwordHash !== pwHash) return { success: false, error: 'Incorrect password.' };
        if (foundUser.isBlocked) return { success: false, error: 'This account has been blocked. Contact support.' };
        if (foundUser.isSuspended) return { success: false, error: 'This account is suspended. Contact support.' };

        const updatedUser = { ...foundUser, lastLogin: new Date().toISOString() };
        const token = `xc-token-${Date.now()}`;
        set({
          registeredUsers: state.registeredUsers.map(u => u.id === foundUser.id ? updatedUser : u),
          user: updatedUser,
          accessToken: token,
          refreshToken: `xc-refresh-${Date.now()}`,
          isAuthenticated: true,
      ...
      updateUserById: (userId, updates) => {
        set((state) => ({
          registeredUsers: state.registeredUsers.map(u => u.id === userId ? { ...u, ...updates } : u),
      ...
      createUserAsAdmin: async ({ firstName, lastName, email, password, role = 'USER', tier = 'CORE' }) => {
        const state = get();
        const existing = state.registeredUsers.find(u => u.email.toLowerCase() === email.toLowerCase());
        if (existing) return { success: false, error: 'An account with this email already exists.' };
