...
export default function AIOracle({
  ...
  const allocData = Object.entries(allocation).map(([name, value]) => ({
    name,
    value: Number(value),
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-xc-dark/40 rounded-xl p-4 border border-xc-border/60">
        <div className="text-xs font-bold text-xc-muted mb-3 uppercase tracking-wider">
          Optimal Allocation
        </div>
        {loading ? (
        ...
        <div className="space-y-2">
          ...
          {forecasts.map((fc) => {
            ...
            return (
              <div
                key={fc.symbol}
                className="flex items-center justify-between p-3.5 rounded-xl bg-xc-dark/40 border border-xc-border/60 hover:border-xc-border transition-all"
              >
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    >
                      ...
                      {fc.expectedReturn >= 0 ? "+" : ""}
                      {formatPercent(fc.expectedReturn)}
                    </div>
                    <div className="text-xs text-xc-muted">
                      → {formatCurrency(fc.predictedPrice)}
                    </div>
                  </div>
                  ...
                  {/* Signal badge */}
                  <Badge variant={sig.variant} size="sm">
                    {fc.signal}
                  </Badge>
                </div>
              </div>
      ...
      <p className="text-[10px] text-xc-muted/60 leading-relaxed">
        AI Oracle predictions are generated by machine learning models and do
        not constitute financial advice. Past performance of AI forecasts does
        not guarantee future accuracy. Always conduct your own research before
        investing.
      </p>
    </div>
  );
