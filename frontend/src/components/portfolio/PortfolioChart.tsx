  ...
  if (active && payload?.length) {
    return (
      <div className="bg-xc-card border border-xc-border rounded-lg px-3 py-2 shadow-xl">
        ...
        <p className="text-sm font-mono font-bold text-white">{formatCurrency(payload[0].value)}</p>
      </div>
    );
  }
  return null;
...
export default function PortfolioChart({ data, height = 200 }: PortfolioChartProps) {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className={cn('text-sm font-bold', getChangeColor(pctChange))}>
          {positive ? '+' : ''}{formatPercent(pctChange)}
          <span className="text-xc-muted font-normal ml-1.5">({activePeriod})</span>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={filtered} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
            ...
            <Area
              ...
              fill="url(#portGrad)"
              dot={false}
              activeDot={{ r: 4, fill: positive ? '#10b981' : '#ef4444', strokeWidth: 0 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      </div>
