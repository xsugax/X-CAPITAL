...
import {
  AreaChart, Area, BarChart, Bar, ResponsiveContainer, Tooltip, XAxis, YAxis, Cell, ScatterChart, Scatter, ZAxis,
} from 'recharts';
import {
  TrendingUp, Lock, DollarSign, AlertCircle, CheckCircle2, BarChart3, Target, Flame, Zap,
} from 'lucide-react';
...
function generateFundPerformance(funds: typeof DEMO_FUNDS) {
  ...
  return months.map((m) => {
    ...
    funds.slice(0, 6).forEach((f) => {
      entry[f.name.split(' ').slice(0, 2).join(' ')] = Number(((Math.random() - 0.3) * 15).toFixed(2));
    });
    return entry;
  });
}

const FUND_COLORS = ['#7c3aed', '#06b6d4', '#d97706', '#10b981', '#ef4444', '#a78bfa'];
...
export default function FundsPage() {
  ...
  const [aumHistory, setAumHistory] = useState<Array<{ month: string; aum: number }>>([]);
  const [perfData, setPerfData] = useState<Array<Record<string, string | number>>>([]);
  ...
  useEffect(() => {
    const totalAUM = displayFunds.reduce((s, f) => s + Number(f.currentAUM ?? 0), 0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setAumHistory((prev) =>
        prev.map((d, i) => i === prev.length - 1 ? { ...d, aum: d.aum * (1 + (Math.random() - 0.48) * 0.002) } : d)
      );
    }, 4000);
  ...
  const scatterData = displayFunds.map((f) => ({
    ...
    risk: f.riskLevel === 'HIGH' ? 75 + Math.random() * 20 : f.riskLevel === 'MEDIUM' ? 35 + Math.random() * 25 : 10 + Math.random() * 20,
    return: Number(f.targetReturn),
    aum: Number(f.currentAUM ?? 0),
  }));

  return (
    <DashboardLayout title="Funds &amp; SPVs" subtitle="Institutional-grade private investments · Live AUM tracking">
      <div className="space-y-8">

        {/* ── Stats ── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Platform AUM" value={formatCurrency(totalAUM)} icon={<DollarSign className="w-5 h-5" />} />
          <StatCard title="Your Deployed" value={formatCurrency(totalInvested)} icon={<TrendingUp className="w-5 h-5" />} />
          <StatCard title="Active Positions" value={String(myInvestments.filter(i => i.status === 'ACTIVE').length)} icon={<Target className="w-5 h-5" />} />
        ...
        <div className="bg-xc-card border border-xc-border rounded-2xl p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-black text-white text-lg">Platform AUM Growth</h3>
              <p className="text-sm text-xc-muted mt-1">12-month assets under management · Live</p>
            </div>
            <div className="text-right">
              <div className="text-2xl font-black text-white font-mono">{formatCurrency(totalAUM)}</div>
              <div className="text-sm text-emerald-400 font-bold">+142% YoY</div>
            </div>
          </div>
          <div style={{ height: 280 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={aumHistory} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                <defs>
                ...
                <Tooltip contentStyle={{ background: '#0d0d1e', border: '1px solid #1a1a3a', borderRadius: 8, fontSize: 12 }} formatter={(v: number) => [formatCurrency(v), 'AUM']} />
                <Area type="monotone" dataKey="aum" stroke="#7c3aed" strokeWidth={2.5} fill="url(#aumGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-xc-card border border-xc-border rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-[10px] text-xc-muted ml-auto">All funds</span>
            </div>
            <div style={{ height: 280 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
                  <XAxis type="number" dataKey="risk" name="Risk" tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} label={{ value: 'Risk Score', position: 'bottom', fill: '#64748b', fontSize: 10, offset: -5 }} />
                  <YAxis type="number" dataKey="return" name="Return" tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} label={{ value: 'Target Return %', angle: -90, position: 'insideLeft', fill: '#64748b', fontSize: 10 }} />
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number, name: string) => [name === 'Return' ? `${v}%` : name === 'Risk' ? v.toFixed(0) : formatCurrency(v), name]}
          ...
          <div className="bg-xc-card border border-xc-border rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="font-bold text-white">Fund Performance Comparison</h3>
              <span className="text-[10px] text-xc-muted ml-auto">6 months</span>
            </div>
            <div style={{ height: 280 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={perfData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                  <XAxis dataKey="month" tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} />
                  <Tooltip contentStyle={{ background: '#0d0d1e', border: '1px solid #1a1a3a', borderRadius: 8, fontSize: 11 }} />
                  {displayFunds.slice(0, 6).map((f, i) => (
                    <Bar key={f.id} dataKey={f.name.split(' ').slice(0, 2).join(' ')} fill={FUND_COLORS[i]} opacity={0.7} radius={[2, 2, 0, 0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="flex flex-wrap gap-3 mt-4">
              {displayFunds.slice(0, 6).map((f, i) => (
                <div key={f.id} className="flex items-center gap-1.5 text-[10px] text-xc-muted">
                  <div className="w-2 h-2 rounded-full" style={{ background: FUND_COLORS[i] }} />
        ...
        {myInvestments.length > 0 && (
          <div className="bg-xc-card border border-xc-border rounded-2xl p-6">
            <h3 className="font-bold text-white mb-5 text-lg">My Investments</h3>
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-sm text-xc-muted">{displayFunds.length} funds open for investment</span>
          </div>
          {loading ? (
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
              {[1,2,3,4,5,6].map(i => <div key={i} className="h-80 rounded-2xl bg-xc-card border border-xc-border animate-pulse" />)}
            </div>
          ) : (
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" value={investAmount} onChange={(e) => setInvestAmount(e.target.value)}
                  min={Number(selected.minInvestment)} step="100"
