...
import {
  Car, Home, Gem, Briefcase, Smartphone, DollarSign,
  ArrowRight, Zap, Globe, Brain, Wifi, Shield, Star,
  ChevronRight, Lock, Users, TrendingUp, Radio,
  CheckCircle2, Clock, Wallet, RefreshCw, Cpu, Activity,
  Server, BarChart3, Eye, CreditCard, CircleDot,
} from 'lucide-react';
...
const ENGINE_NODES = [
  ...
  { label: 'AI Inference Layer',  icon: Brain,    desc: 'Compute + bandwidth routing' },
  { label: 'Global Payments',     icon: Globe,    desc: 'Revenue distribution network' },
];
...
const TIERS = [
  {
    ...
    price: '$500',
    yield: '$2,400/mo',
    features: ['Starlink bandwidth routing', 'Basic AI compute allocation', 'Monthly yield distribution', 'Hand-Held Digital Wallet', 'Community access'],
    highlight: false,
  },
  {
    name: 'Relay',
    price: '$2,500',
    yield: '$14,000/mo',
    features: ['Priority bandwidth reserves', 'Full AI inference layer', 'Weekly yield distribution', 'Asset bridge access', 'Elevation sequence eligible', 'Premium support'],
    highlight: true,
  },
  {
    name: 'Vanguard',
    price: '$10,000',
    yield: '$62,000/mo',
    features: ['Dedicated compute cluster', 'Maximum bandwidth allocation', 'Daily yield distribution', 'White-glove acquisition', 'Priority elevation path', 'Direct partner network', 'Founder-tier governance'],
    highlight: false,
  },
];
...
const YIELD_FEED_SYMBOLS = ['TSLA-NODE', 'RE-NYC', 'GPU-CLUSTER', 'STRLINK-12', 'AI-INFER', 'LUX-VAULT', 'BIZ-ASSET', 'FIN-ROUTE', 'ELEC-MESH', 'RE-LONDON', 'TSLA-RELAY', 'GPU-FARM'];

export default function EnginePage() {
  const [selectedAsset, setSelectedAsset] = useState<string | null>(null);
  const [engineSyncing, setEngineSyncing] = useState(false);
  const [activeTier, setActiveTier] = useState<number | null>(null);
  const [yieldFeed, setYieldFeed] = useState<Array<{ id: string; sym: string; yield: string; time: string; status: string }>>([]);

  // Live yield feed
  useEffect(() => {
    const push = () => {
      const sym = YIELD_FEED_SYMBOLS[Math.floor(Math.random() * YIELD_FEED_SYMBOLS.length)];
  ...
  return (
    <DashboardLayout title="The Engine" subtitle="Turn any liability into a revenue node">
      <div className="space-y-0">

        {/* ── HERO: AI COMPUTE + BANDWIDTH ──────────────────────────── */}
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <div className="max-w-3xl">
            <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">X-CAPITAL INFRASTRUCTURE</p>
            <h1 className="text-4xl md:text-5xl font-black text-white leading-[1.1] tracking-tight mb-6">
              AI Compute Layer.<br />Bandwidth Reserves.<br />
              <span className="text-white/40">Structural Yield.</span>
            </h1>
            <p className="text-base text-white/40 leading-relaxed max-w-xl mb-10">
              Every node in the X-CAPITAL network routes real bandwidth through Starlink infrastructure,
              processes AI inference workloads, and distributes revenue globally. The yield isn&apos;t speculative &mdash;
              it&apos;s structural.
            </p>
          </div>
        ...
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">YIELD ARCHITECTURE</p>
          <h2 className="text-3xl font-black text-white tracking-tight mb-3">Why the yield is structurally guaranteed.</h2>
          <p className="text-sm text-white/35 max-w-2xl mb-10">
            Revenue flows from three convergent infrastructure layers. Each generates independently.
        ...
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-2">LIVE FEED</p>
              <h2 className="text-2xl font-black text-white tracking-tight">Global Yield Feed</h2>
            </div>
          ...
          <div className="border border-white/[0.06] rounded-2xl overflow-hidden">
            ...
            {/* Feed */}
            <div style={{ height: 360, overflowY: 'auto' }} className="custom-scrollbar">
              {yieldFeed.map((item, i) => (
                ...
                >
                  ...
                  <span className="font-mono text-white/70 text-right">{item.yield}</span>
                  <span className="font-mono text-white/25 text-right">{item.time}</span>
                  <span className={cn(
                    'text-right font-mono text-[11px]',
                    item.status === 'settled' ? 'text-white/50' : 'text-white/25'
                  )}>
                    {item.status === 'settled' ? '\u2713 Settled' : '\u25CB Routing'}
                  </span>
                </div>
              ))}
            </div>
        ...
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">ASSET INTEGRATION</p>
          <h2 className="text-3xl font-black text-white tracking-tight mb-3">Turn any liability into a vault.</h2>
          <p className="text-sm text-white/35 max-w-xl mb-10">Select your asset class. The engine syncs.</p>
          ...
          <div className="grid md:grid-cols-[280px_1fr] gap-10">
            ...
            <div className="border border-white/[0.08] rounded-2xl overflow-hidden">
              {ASSET_CATEGORIES.map((cat) => (
                ...
                >
                  <cat.icon className={cn('w-5 h-5 shrink-0', selectedAsset === cat.id ? 'text-white' : 'text-white/20')} />
                  <div>
                    <div className={cn('text-sm font-bold', selectedAsset === cat.id ? 'text-white' : 'text-white/60')}>
                      {cat.label}
                    </div>
                    <div className="text-[11px] text-white/20">{cat.example}</div>
                  </div>
                  {selectedAsset === cat.id && <ChevronRight className="w-4 h-4 text-white/40 ml-auto" />}
                </button>
              ))}
            </div>
            ...
            <div className="border border-white/[0.08] rounded-2xl p-8">
              <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-white/25 mb-8">GLOBAL ROUTING ENGINE</p>
                ...
                {ENGINE_NODES.map((node, i) => (
                  <div key={node.label} className="w-full">
                    ...
                    <div className={cn(
                      ...
                      engineSyncing && i === 0 && selectedAsset && 'border-white/30 bg-white/[0.06]',
                    )}>
                      <div className={cn(
                        ...
                        <node.icon className={cn(
                          ...
                          engineSyncing && selectedAsset ? 'text-white' : 'text-white/25'
                        )} />
                      </div>
                      <div>
                        <div className={cn(
                          'text-sm font-bold transition-colors duration-500',
                          engineSyncing && selectedAsset ? 'text-white' : 'text-white/50'
                        )}>
                          {i === 0 && selectedAsset
                            ? ASSET_CATEGORIES.find(c => c.id === selectedAsset)?.label?.toUpperCase() ?? node.label
                            : node.label}
                        </div>
                        <div className="text-[11px] text-white/20">{node.desc}</div>
                      </div>
                      {engineSyncing && (
                    ...
                    {i < ENGINE_NODES.length - 1 && (
                      <div className="flex justify-center py-2">
                        <div className={cn(
                          ...
                          engineSyncing && selectedAsset ? 'bg-white/30' : 'bg-white/[0.06]'
                        )} />
                      </div>
                    )}
                  </div>
                ...
                <div className="mt-8 pt-6 border-t border-white/[0.06] flex items-center gap-3">
                  {engineSyncing ? (
                    <>
                      ...
                      <span className="text-sm text-white/40 font-mono">Syncing engine to {ASSET_CATEGORIES.find(c => c.id === selectedAsset)?.label}...</span>
                    </>
                  ) : (
                    <>
                      <CheckCircle2 className="w-4 h-4 text-white/60" />
                      <span className="text-sm text-white/40 font-mono">Engine linked. {ASSET_CATEGORIES.find(c => c.id === selectedAsset)?.label} routed to yield pool.</span>
                    </>
                  )}
                </div>
              )}
        ...
        {/* ── THE PROCESS ───────────────────────────────────────────── */}
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">THE PROCESS</p>
          <h2 className="text-3xl font-black text-white tracking-tight mb-12">How any asset becomes your engine.</h2>
        ...
        {/* ── ARCHITECTURE (01-04) ──────────────────────────────────── */}
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">SYSTEM DESIGN</p>
          <h2 className="text-3xl font-black text-white tracking-tight mb-12">Structural architecture.</h2>
        ...
        <section className="border-b border-white/[0.06] pb-16 mb-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-2">VERIFIED REVIEWS</p>
              <h2 className="text-2xl font-black text-white tracking-tight">Trusted by integrators worldwide.</h2>
            </div>
            ...
            {REVIEWS.map((review) => (
              <div key={review.name} className="bg-[#080812] p-6">
                <div className="flex mb-3">
                  {[...Array(review.rating)].map((_, i) => <Star key={i} className="w-3 h-3 fill-white/60 text-white/60" />)}
                </div>
        ...
        <section className="pb-16">
          <div className="text-center mb-12">
            <p className="text-xs font-mono uppercase tracking-[0.2em] text-white/30 mb-4">ACTIVATE</p>
            <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight mb-3">Activate your revenue node.</h2>
            <p className="text-sm text-white/35 max-w-lg mx-auto">Three tiers. One engine. Choose your deployment level.</p>
          </div>
