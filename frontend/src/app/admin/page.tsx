...
const SYSTEM_METRICS = {
  uptime: "99.97%", latency: "42ms", activeConnections: 1847,
  cpuUsage: 34, memoryUsage: 61, diskUsage: 28,
  apiCalls24h: 2_450_000, errorRate: "0.03%",
  lastDeploy: "2026-04-02T14:30:00Z", version: "3.8.1", nodeCount: 12,
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard icon={DollarSign} label="Total AUM" value={formatCurrency(totalAUM)} change={`${users.length} accounts`} positive />
        <KPICard icon={Users} label="Total Users" value={users.length.toString()} change={`${activeUsers} active`} positive />
        <KPICard icon={Activity} label="System Uptime" value={SYSTEM_METRICS.uptime} change={SYSTEM_METRICS.latency + " latency"} positive />
        <KPICard icon={AlertTriangle} label="Flagged" value={(blockedUsers + suspendedUsers + frozenUsers).toString()} change={blockedUsers > 0 ? "Action needed" : "All clear"} positive={blockedUsers === 0} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <AdminCard title="Recent Activity" icon={Clock}>
            <div className="space-y-2">
              {(auditLog || []).length === 0 ? (
                <p className="text-xc-muted text-sm py-4 text-center">No audit entries yet. Admin actions will appear here.</p>
              ) : (
                (auditLog || []).slice(0, 8).map((entry, i) => (
                  <div key={entry.id || i} className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
                    <div className="flex items-center gap-3">
                      <div className={cn("w-2 h-2 rounded-full flex-shrink-0",
                        entry.level === "danger" ? "bg-red-500" :
                        entry.level === "warning" ? "bg-amber-500" :
                        entry.level === "success" ? "bg-xc-green" :
                        entry.level === "action" ? "bg-xc-purple" : "bg-white/20"
                      )} />
                      <div>
                        <div className="text-sm text-white">{entry.action}</div>
                        <div className="text-xs text-xc-muted">{entry.target} · by {entry.actor}</div>
                      </div>
                    </div>
                    <span className="text-xs text-xc-muted font-mono">{entry.time}</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="space-y-6">
          ...
          <AdminCard title="User Status" icon={Users}>
            <div className="space-y-3">
              {[
                { label: "Active", count: activeUsers, total: users.length, color: "bg-xc-green" },
                { label: "Frozen", count: frozenUsers, total: users.length, color: "bg-cyan-400" },
                { label: "Suspended", count: suspendedUsers, total: users.length, color: "bg-amber-500" },
                { label: "Blocked", count: blockedUsers, total: users.length, color: "bg-red-500" },
                { label: "Trading Off", count: tradingDisabled, total: users.length, color: "bg-rose-500" },
                ...
                <div key={label}>
                  ...
                  <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <div className={cn("h-full rounded-full transition-all", color)} style={{ width: total > 0 ? `${(count / total) * 100}%` : '0%' }} />
                  </div>
                </div>
              ))}
            </div>
...
function UsersTab() {
  const { registeredUsers, user: currentAdmin, updateUserById, deleteUserById, addAuditEntry } = useStore();
  const users = registeredUsers || [];
  ...
  const filtered = useMemo(() => {
    return users.filter((u) => {
      const matchSearch = !search || u.firstName.toLowerCase().includes(search.toLowerCase()) || u.lastName.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase());
      ...
      if (filterStatus === "TRADING_OFF") return matchSearch && !u.tradingEnabled;
      if (filterStatus === "ACTIVE") return matchSearch && !u.isBlocked && !u.isSuspended;
      return matchSearch;
    });
  ...
  const toggleFreeze = (u: User) => {
    ...
    audit(u.isFrozen ? "Unfroze account" : "Froze account", `${u.firstName} ${u.lastName}`, u.isFrozen ? "success" : "warning");
    showToast(`${u.firstName} ${u.lastName} ${u.isFrozen ? "unfrozen" : "frozen"}`);
  };
  const toggleSuspend = (u: User) => {
    updateUserById(u.id, { isSuspended: !u.isSuspended });
    audit(u.isSuspended ? "Reactivated account" : "Suspended account", `${u.firstName} ${u.lastName}`, u.isSuspended ? "success" : "warning");
    showToast(`${u.firstName} ${u.lastName} ${u.isSuspended ? "reactivated" : "suspended"}`);
  };
  const toggleBlock = (u: User) => {
    updateUserById(u.id, { isBlocked: !u.isBlocked });
    audit(u.isBlocked ? "Unblocked account" : "Blocked account", `${u.firstName} ${u.lastName}`, u.isBlocked ? "success" : "danger");
    showToast(`${u.firstName} ${u.lastName} ${u.isBlocked ? "unblocked" : "blocked"}`);
  };
  const toggleTrading = (u: User) => {
    updateUserById(u.id, { tradingEnabled: !u.tradingEnabled });
    audit(u.tradingEnabled ? "Stopped trading" : "Started trading", `${u.firstName} ${u.lastName}`, "action");
    showToast(`Trading ${u.tradingEnabled ? "stopped" : "started"} for ${u.firstName}`);
  };
  const toggleProfitHold = (u: User) => {
    updateUserById(u.id, { profitHold: !u.profitHold });
    audit(u.profitHold ? "Released profits" : "Held profits", `${u.firstName} ${u.lastName}`, "action");
    showToast(`Profits ${u.profitHold ? "released" : "on hold"} for ${u.firstName}`);
  };
  const saveBalance = (u: User) => {
    const val = parseFloat(balanceInput);
    if (isNaN(val) || val < 0) { showToast("Invalid balance"); return; }
    updateUserById(u.id, { balance: val });
    audit(`Set balance to ${formatCurrency(val)}`, `${u.firstName} ${u.lastName}`, "action");
    showToast(`Balance updated to ${formatCurrency(val)}`);
    setEditingBalance(null);
  };
  const saveMultiplier = (u: User) => {
    const val = parseFloat(multiplierInput);
    if (isNaN(val) || val < 0 || val > 100) { showToast("Invalid multiplier (0-100)"); return; }
    updateUserById(u.id, { profitMultiplier: val });
    audit(`Set profit multiplier to ${val}x`, `${u.firstName} ${u.lastName}`, "action");
  ...
  const handleDelete = (u: User) => {
    deleteUserById(u.id);
    audit("Deleted account", `${u.firstName} ${u.lastName} (${u.email})`, "danger");
    ...
    <div className="space-y-4 relative">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {["ALL", "ACTIVE", "FROZEN", "SUSPENDED", "BLOCKED", "TRADING_OFF"].map((f) => (
            <button key={f} onClick={() => setFilterStatus(f)}
              className={cn("px-3 py-1.5 rounded-full text-xs font-bold transition-all",
                filterStatus === f ? "bg-xc-purple text-white" : "text-xc-muted hover:text-white bg-white/[0.04]"
              )}>
              {f.replace(/_/g, " ")}
            </button>
          ))}
      ...
      <div className="text-xs text-xc-muted">
        {filtered.length} user{filtered.length !== 1 ? "s" : ""} · Total AUM: <span className="text-white font-bold">{formatCurrency(filtered.reduce((s, u) => s + (u.balance || 0), 0))}</span>
      </div>
      ...
      <div className="bg-xc-card border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="hidden lg:grid lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] gap-3 px-5 py-3 border-b border-white/[0.06] text-xs font-bold text-xc-muted uppercase tracking-wider">
          <span>User</span><span>Status</span><span>Tier / Role</span><span>Balance</span><span>Trading</span><span>Joined</span>
        </div>

        {filtered.length === 0 ? (
          <div className="py-12 text-center text-xc-muted text-sm">No users found.</div>
        ) : filtered.map((u) => (
          <div key={u.id}>
            {/* User row */}
            <div className="grid grid-cols-[1fr_auto] lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] gap-3 px-5 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors cursor-pointer items-center"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-bold text-white truncate">{u.firstName} {u.lastName}</div>
                  <div className="text-xs text-xc-muted truncate">{u.email}</div>
                </div>
              </div>
              <div className="hidden lg:flex flex-wrap gap-1">
                {u.isBlocked && <MicroBadge color="bg-red-500/20 text-red-400">BLOCKED</MicroBadge>}
                {u.isSuspended && !u.isBlocked && <MicroBadge color="bg-amber-500/20 text-amber-400">SUSPENDED</MicroBadge>}
                {u.isFrozen && <MicroBadge color="bg-cyan-500/20 text-cyan-400">FROZEN</MicroBadge>}
                {!u.isBlocked && !u.isSuspended && !u.isFrozen && <MicroBadge color="bg-emerald-500/20 text-xc-green">ACTIVE</MicroBadge>}
              </div>
              <div className="hidden lg:flex flex-col gap-0.5">
                <TierBadge tier={u.tier} />
                {u.role !== "USER" && <span className="text-[10px] font-mono text-red-400">{u.role}</span>}
              </div>
              <div className="hidden lg:block text-sm font-mono font-bold text-white">{formatCurrency(u.balance || 0)}</div>
              <div className="hidden lg:block">
                {u.tradingEnabled ? (
                  <span className="text-xs text-xc-green font-semibold flex items-center gap-1"><Play className="w-3 h-3" /> ON</span>
                ) : (
                  <span className="text-xs text-red-400 font-semibold flex items-center gap-1"><Pause className="w-3 h-3" /> OFF</span>
                )}
              </div>
              <div className="hidden lg:block text-xs text-xc-muted">{new Date(u.createdAt).toLocaleDateString()}</div>
              <ChevronDown className={cn("w-4 h-4 text-xc-muted transition-transform lg:hidden", expandedUser === u.id && "rotate-180")} />
            </div>
            ...
            {expandedUser === u.id && (
              <div className="px-5 py-5 bg-black/30 border-b border-white/[0.06]">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                  ...
                  <DetailItem label="KYC" value={(u.kycStatus || "NOT_STARTED").replace(/_/g, " ")} />
                  <DetailItem label="Last Login" value={u.lastLogin ? timeAgo(u.lastLogin) : "Never"} />
                  <DetailItem label="Trades" value={String(u.trades || 0)} />
                  <DetailItem label="Country" value={u.country || "N/A"} />
                  <DetailItem label="Profit Multiplier" value={`${u.profitMultiplier || 1}x`} />
                  <DetailItem label="Profits" value={u.profitHold ? "ON HOLD" : "RELEASING"} />
                </div>

                {/* Admin Power Controls */}
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    <AdminBtn icon={Snowflake} label={u.isFrozen ? "Unfreeze" : "Freeze"} onClick={() => toggleFreeze(u)} variant={u.isFrozen ? "success" : "default"} />
                    <AdminBtn icon={u.isSuspended ? Unlock : Lock} label={u.isSuspended ? "Reactivate" : "Suspend"} onClick={() => toggleSuspend(u)} variant={u.isSuspended ? "success" : "danger"} />
                    <AdminBtn icon={Ban} label={u.isBlocked ? "Unblock" : "Block"} onClick={() => toggleBlock(u)} variant={u.isBlocked ? "success" : "danger"} />
                    <AdminBtn icon={u.tradingEnabled ? Pause : Play} label={u.tradingEnabled ? "Stop Trading" : "Start Trading"} onClick={() => toggleTrading(u)} variant={u.tradingEnabled ? "danger" : "success"} />
                    <AdminBtn icon={u.profitHold ? TrendingUp : TrendingDown} label={u.profitHold ? "Release Profits" : "Hold Profits"} onClick={() => toggleProfitHold(u)} variant={u.profitHold ? "success" : "default"} />
                    {u.kycStatus !== "APPROVED" && (
                      <AdminBtn icon={CheckCircle} label="Approve KYC" onClick={() => approveKYC(u)} variant="success" />
                    )}
                  </div>

                  {/* Balance Control */}
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xs text-xc-muted font-semibold w-20">Balance:</span>
                    {editingBalance === u.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => saveBalance(u)} className="px-3 py-1.5 rounded-lg bg-xc-green/20 text-xc-green text-xs font-bold hover:bg-xc-green/30">Save</button>
                        <button onClick={() => setEditingBalance(null)} className="px-3 py-1.5 rounded-lg bg-white/[0.04] text-xc-muted text-xs font-bold hover:bg-white/[0.08]">Cancel</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-mono font-bold text-white">{formatCurrency(u.balance || 0)}</span>
                        <button onClick={() => { setEditingBalance(u.id); setBalanceInput(String(u.balance || 0)); }}
                          className="text-xs text-xc-purple-light hover:text-white transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Profit Multiplier */}
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xs text-xc-muted font-semibold w-20">Profit ×:</span>
                    {editingMultiplier === u.id ? (
                      <div className="flex items-center gap-2">
                        <input type="number" step="0.1" value={multiplierInput} onChange={(e) => setMultiplierInput(e.target.value)}
                        ...
                        <button onClick={() => saveMultiplier(u)} className="px-3 py-1.5 rounded-lg bg-xc-green/20 text-xc-green text-xs font-bold hover:bg-xc-green/30">Save</button>
                        <button onClick={() => setEditingMultiplier(null)} className="px-3 py-1.5 rounded-lg bg-white/[0.04] text-xc-muted text-xs font-bold hover:bg-white/[0.08]">Cancel</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-mono font-bold text-white">{u.profitMultiplier || 1}x</span>
                        <button onClick={() => { setEditingMultiplier(u.id); setMultiplierInput(String(u.profitMultiplier || 1)); }}
                          className="text-xs text-xc-purple-light hover:text-white transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-xc-muted font-semibold">Tier:</span>
                      <select value={u.tier} onChange={(e) => changeTier(u, e.target.value)}
                        ...
                        <option value="CORE">CORE</option><option value="GOLD">GOLD</option><option value="BLACK">BLACK</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-xc-muted font-semibold">Role:</span>
                      <select value={u.role || "USER"} onChange={(e) => changeRole(u, e.target.value)}
                        className="bg-xc-dark border border-white/[0.1] rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none">
                        <option value="USER">USER</option><option value="ADMIN">ADMIN</option><option value="GOD_ADMIN">GOD_ADMIN</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-white/[0.06]">
                    {confirmDelete === u.id ? (
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-red-400 font-semibold">Permanently delete this account?</span>
                        <button onClick={() => handleDelete(u)} className="px-4 py-1.5 rounded-lg bg-red-600 text-white text-xs font-bold hover:bg-red-500">Yes, Delete</button>
                        <button onClick={() => setConfirmDelete(null)} className="px-4 py-1.5 rounded-lg bg-white/[0.06] text-xc-muted text-xs font-bold hover:bg-white/[0.1]">Cancel</button>
                      </div>
                    ) : (
                      <AdminBtn icon={Trash2} label="Delete Account" onClick={() => setConfirmDelete(u.id)} variant="danger" />
                    )}
                  </div>
                </div>
              </div>
...
function CreateUserTab() {
  ...
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", password: "", role: "USER", tier: "CORE" });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setForm({ firstName: "", lastName: "", email: "", password: "", role: "USER", tier: "CORE" });
    } else {
      setResult({ success: false, message: res.error || "Failed to create account." });
    }
  };
      ...
      <AdminCard title="Create New Account" icon={UserPlus}>
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1">First Name</label>
            ...
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1">Last Name</label>
              <input type="text" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })}
                className="w-full bg-xc-dark border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white focus:ring-1 focus:ring-xc-purple/50 focus:outline-none" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-xc-muted mb-1">Email</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
              className="w-full bg-xc-dark border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white focus:ring-1 focus:ring-xc-purple/50 focus:outline-none" />
          </div>
          <div>
            <label className="block text-xs font-medium text-xc-muted mb-1">Password</label>
            <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
              className="w-full bg-xc-dark border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white focus:ring-1 focus:ring-xc-purple/50 focus:outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1">Role</label>
            ...
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1">Tier</label>
          ...
          {result && (
            <div className={cn("px-4 py-3 rounded-xl text-sm font-semibold",
              result.success ? "bg-emerald-950/40 text-xc-green border border-emerald-800/30" : "bg-red-950/40 text-red-400 border border-red-800/30"
            )}>
              {result.message}
            </div>
          )}
      ...
      <AdminCard title="Platform Controls" icon={Power}>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            ...
            { icon: Globe, label: "Sync All Users", desc: "Force-sync all user sessions" },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-xc-muted">{entries.length} audit entries recorded</p>
      </div>
      <div className="bg-xc-card border border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        {entries.length === 0 ? (
          <div className="py-12 text-center text-xc-muted text-sm">No audit entries yet. Perform admin actions to see them here.</div>
        ) : (
          entries.map((entry, i) => (
            <div key={entry.id || i} className="grid grid-cols-[1fr_auto] md:grid-cols-[80px_1.5fr_2fr_1.5fr_60px] gap-4 px-5 py-3 border-b border-white/[0.04] hover:bg-white/[0.02] items-center">
...
function AdminCard({ title, icon: Icon, badge, children }: { title: string; icon: React.ElementType; badge?: string; children: React.ReactNode }) {
  return (
    <div className="bg-xc-card border border-white/[0.06] rounded-2xl p-5">
...
function AdminBtn({ icon: Icon, label, variant = "default", onClick }: { icon: React.ElementType; label: string; variant?: "default" | "success" | "danger"; onClick?: () => void }) {
  const colors = {
  ...
  return (
    <button onClick={onClick} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all", colors[variant])}>
...
function MicroBadge({ color, children }: { color: string; children: React.ReactNode }) {
  return <span className={cn("text-[9px] font-bold px-1.5 py-0.5 rounded-full", color)}>{children}</span>;
}
