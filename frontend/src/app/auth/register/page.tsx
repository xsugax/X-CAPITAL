...
export default function RegisterPage() {
  ...
  const PW_COLORS = ['bg-xc-red', 'bg-amber-500', 'bg-amber-400', 'bg-xc-green', 'bg-xc-green'];
    ...
    <div className="min-h-screen bg-xc-black flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xc-muted text-sm">Create your account — free forever</p>
        </div>
        ...
        <div className="bg-xc-card border border-xc-border rounded-2xl p-8 shadow-2xl shadow-black/60">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-xc-muted mb-1.5">First Name</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-xc-muted" />
                  <input type="text" value={form.firstName} onChange={set('firstName')} placeholder="Jane" required autoComplete="given-name"
              ...
              <div>
                <label className="block text-xs font-medium text-xc-muted mb-1.5">Last Name</label>
                <input type="text" value={form.lastName} onChange={set('lastName')} placeholder="Smith" required autoComplete="family-name"
            ...
            {/* Email */}
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1.5">Email Address</label>
              <div className="relative">
            ...
            {/* Password */}
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1.5">Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-xc-muted" />
                <input type={showPw ? 'text' : 'password'} value={form.password} onChange={set('password')} placeholder="Min 8 characters" required autoComplete="new-password"
              ...
              {form.password.length > 0 && (
                <div className="flex gap-1 mt-2">
                  {[0,1,2,3].map(i => (
                    <div key={i} className={`flex-1 h-1 rounded-full ${i < pwStrength ? PW_COLORS[pwStrength] : 'bg-white/10'}`} />
                  ))}
                </div>
              )}
            </div>
            ...
            {/* Confirm password */}
            <div>
              <label className="block text-xs font-medium text-xc-muted mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-xc-muted" />
                <input type={showPw ? 'text' : 'password'} value={form.confirmPassword} onChange={set('confirmPassword')} placeholder="Repeat password" required autoComplete="new-password"
                  className="w-full bg-xc-dark/60 border border-xc-border rounded-xl pl-10 pr-10 py-3 text-sm text-white placeholder:text-xc-muted/50 focus:outline-none focus:border-xc-purple/60 transition-colors" />
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={form.agreeTerms} onChange={set('agreeTerms')} className="mt-0.5 accent-purple-600" />
              <span className="text-xs text-xc-muted leading-relaxed">
                I agree to X-CAPITAL&apos;s{' '}
                <span className="text-xc-purple-light hover:text-white cursor-pointer transition-colors">Terms of Service</span>
                {' '}and{' '}
                <span className="text-xc-purple-light hover:text-white cursor-pointer transition-colors">Privacy Policy</span>.
          ...
          <p className="text-center text-sm text-xc-muted mt-5">
            ...
            <Link href="/auth/login" className="text-xc-purple-light hover:text-white font-semibold transition-colors">
              Sign in
            </Link>
          </p>
        </div>
